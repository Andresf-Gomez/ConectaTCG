...
function PriceTrendChart({ card }: { card: Card }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm mb-5">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-3 mb-5">
        ...
        <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900 rounded-2xl px-4 py-3 text-right">
          <p className="text-xs text-slate-500 dark:text-slate-400">Variación mensual</p>
          <p
  ...
  return (
    <Layout>
      ...
      <div className="grid lg:grid-cols-[360px_1fr] gap-6">
        <div className="bg-gradient-to-br from-blue-50 to-yellow-50 dark:from-blue-950 dark:to-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 flex items-center justify-center min-h-[300px]">
          <CardImage
            src={card.image}
            alt={card.name}
        ...
        <div>
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm mb-5">
            <span className="text-sm font-bold text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/40 px-3 py-1 rounded-full">
              {card.type}
            </span>
            <h2 className="text-4xl font-black text-slate-950 dark:text-white mt-3">
              {card.name}
            </h2>
            <p className="text-slate-500 dark:text-slate-400 mt-1">
              {card.set} · {card.number} · {card.rarity} · {card.language || 'Español'}
            </p>
            <p className="text-slate-700 dark:text-slate-300 mt-4">{card.description}</p>
            <div className="grid sm:grid-cols-3 gap-3 mt-5">
