...
export function HomePage({ setPage, setSelectedCard, searchQuery, setSearchQuery }: { setPage: (page: string) => void; setSelectedCard: (card: Card | GroupedCard) => void; searchQuery: string; setSearchQuery: (q: string) => void }) {
  const [query, setQuery] = useState(searchQuery || '');
  const { cards: supabaseCards } = useCards();
  const featuredCard = supabaseCards[0] ?? null;
  const goSearch = () => {
            ...
            <div className="relative bg-gradient-to-br from-blue-600 to-blue-800 rounded-[3rem] p-6 shadow-2xl overflow-hidden">
              <div className="flex justify-between items-center text-white mb-4">
                <div>
                  ...
                  <h3 className="text-2xl font-black">{featuredCard.name}</h3>
                </div>
                <span className="px-3 py-1 rounded-full bg-yellow-300 text-slate-900 text-sm font-bold">
                  {featuredCard.offers.length} vendedor{featuredCard.offers.length !== 1 ? 'es' : ''}
                </span>
              </div>
